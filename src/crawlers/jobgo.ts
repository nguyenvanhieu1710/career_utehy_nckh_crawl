...
export class JobGoCrawler {
  ...
  static async crawl(): Promise<CompanyInput[]> {
    ...
    try {
      ...
      while (hasNextPage && currentPage <= 2) {
        // Giới hạn 2 trang để test
        this.logger.log(`=== CRAWLING PAGE ${currentPage} ===`);

        await page.goto(
          `https://jobsgo.vn/cong-ty-giao-duc-trang-${currentPage}.html`,
        ...
        if (companyLinks.length === 0) {
          this.logger.log("No companies found on this page, stopping crawl");
          break;
        }

        this.logger.log(
          `Found ${companyLinks.length} companies on page ${currentPage}`
        );
        ...
        for (const link of companyLinks) {
          try {
              ...
              try {
                ...
                const jobData = await page.evaluate(() => {
                  ...
                  if (!skills.length) {
                    ...
                    skills.push(...keywords.filter((kw) => text.includes(kw)));
                  }
                ...
                this.logger.error(`Error crawling job ${jobUrl}: ${jobError}`);
                continue;
              }
            }
            ...
            const finalUniqueLocations = [...new Set(combinedLocations)].filter(
              (loc): loc is string =>
        ...
        const nextPageExists = await page.evaluate((currentPageNum: number) => {
        ...
        if (nextPageExists) {
          currentPage++;
          this.logger.log(`Moving to page ${currentPage}`);
        } else {
          hasNextPage = false;
          this.logger.log("Finished crawling all pages");
        }
      ...
      this.logger.log("\n=== CRAWL COMPLETED ===");
      this.logger.log(`Total pages crawled: ${currentPage - 1}`);
